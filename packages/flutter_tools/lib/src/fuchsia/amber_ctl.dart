...
class FuchsiaAmberCtl {
  ...
  Future<bool> addSrc(FuchsiaDevice device, FuchsiaPackageServer server) async {
    final String localIp = await _getLocalIp(device);
  ...
  Future<String> _getLocalIp(FuchsiaDevice device) async {
    final RunResult result = await device.shell('echo \$SSH_CONNECTION');

    if (result.exitCode != 0) {
      throw Exception('Failed to get local address, aborting.');
    } else {
      return result.stdout.split(' ')[0].replaceAll('%', '%25');
    }
  }
}
